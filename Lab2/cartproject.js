...
const removeProduct = async (pid) => {
  const cart = await getCart();
  let x = cart.length;
  // const isFoundInCart = cart.find((item) => item.id === pid);5
...
const main = async () => {
  ...
  do {
    ...
    switch (Number(choice)) {
      ...
      case 3:
        var pid = await cin.question("Enter product id to be deleted:");
        await removeProduct(Number(pid));
        break;
      case 4:
        let pid2 = await cin.question("Enter product id, option(+/-):");
